...
export async function walkLibrary(root: string): Promise<AlbumLayout[]> {
  ...
  // If root itself contains music, treat it as a single album folder and infer
  // the layout from the path structure (grandparent = letter dir if single char,
  // parent = artist name, basename = album folder).
  if (await hasMusic(root)) {
    const albumFolder = path.basename(root);
    const artistDir = path.dirname(root);
    const artistName = path.basename(artistDir);
    const maybeLetterDir = path.basename(path.dirname(artistDir));
    const letterDir = LETTER_DIR_RE.test(maybeLetterDir) ? maybeLetterDir : undefined;
    layouts.push({ letterDir, artistName, albumFolder, albumPath: root });
    return layouts;
  }
